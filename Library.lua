...
syn = syn or {
	protect_gui = function(UI)
		return;
	end
}

if getgenv().VapeUI then
	pcall(function()
		if typeof(getgenv().VapeUI) == 'Instance' then
			getgenv().VapeUI:Destroy()
		end
	end)
end
...
local Service = setmetatable({}, {
	__index = function(_, Service)
		if Service == 'CoreGui' and RunService:IsStudio() then
...
function lib:SetCloseBind(Bind)
	CloseBind = Bind
end

function lib:ToggleUI(Opened)
	uitoggled = Opened or not uitoggled
	if (Opened or uitoggled) then
		Main:TweenSize(UDim2.new(0, 0, 0, 0), Enum.EasingDirection.Out, Enum.EasingStyle.Quart, .6, true)
		uitoggled = true
		wait(.5)
		ui.Enabled = false
	...
	function tabhold:Tab(text)
		...
		local tabcontent = {}
		function tabcontent:SetTitle(NewTitle)
		...
		function tabcontent:Dropdown(text, list, callback)
			...
			return {
				...
				Set = function(Items)
					itemcount = 0
					DropItemHolder:ClearAllChildren()
		...
		function tabcontent:Bind(text, keypreset, callback)
			...
			return {
				Set = function(NewKeybind)
					if NewKeybind.KeyCode.Name ~= 'Unknown' then
						BindText.Text = NewKeybind.KeyCode.Name
						Key = NewKeybind.KeyCode.Name
						binding = false
					else
						binding = false
					end
